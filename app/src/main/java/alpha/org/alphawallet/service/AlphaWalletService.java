package alpha.org.alphawallet.service;
...
import alpha.org.alphawallet.module.AlphaModuleImp;
import alpha.org.alphawallet.module.store.SnappyBlockchainStore;
import alpha.org.alphawallet.rate.CoinMarketCapApiClient;
import alpha.org.alphawallet.rate.CoinTypes;
import alpha.org.alphawallet.rate.RequestAlphaRateException;
...
import static alpha.org.alphawallet.service.IntentsConstants.ACTION_ADDRESS_BALANCE_CHANGE;
import static alpha.org.alphawallet.service.IntentsConstants.ACTION_BROADCAST_TRANSACTION;
import static alpha.org.alphawallet.service.IntentsConstants.ACTION_CANCEL_COINS_RECEIVED;
import static alpha.org.alphawallet.service.IntentsConstants.ACTION_NOTIFICATION;
import static alpha.org.alphawallet.service.IntentsConstants.ACTION_RESET_BLOCKCHAIN;
import static alpha.org.alphawallet.service.IntentsConstants.ACTION_SCHEDULE_SERVICE;
import static alpha.org.alphawallet.service.IntentsConstants.DATA_TRANSACTION_HASH;
import static alpha.org.alphawallet.service.IntentsConstants.INTENT_BROADCAST_DATA_BLOCKCHAIN_STATE;
import static alpha.org.alphawallet.service.IntentsConstants.INTENT_BROADCAST_DATA_ON_COIN_RECEIVED;
import static alpha.org.alphawallet.service.IntentsConstants.INTENT_BROADCAST_DATA_PEER_CONNECTED;
import static alpha.org.alphawallet.service.IntentsConstants.INTENT_BROADCAST_DATA_TYPE;
import static alpha.org.alphawallet.service.IntentsConstants.INTENT_EXTRA_BLOCKCHAIN_STATE;
import static alpha.org.alphawallet.service.IntentsConstants.NOT_BLOCKCHAIN_ALERT;
import static alpha.org.alphawallet.service.IntentsConstants.NOT_COINS_RECEIVED;

/**
 * Created by kaali on 6/12/17.
 */

public class AlphaWalletService extends Service{

    private Logger log = LoggerFactory.getLogger(AlphaWalletService.class);

    private AlphaApplication alphaApplication;
    private AlphaModuleImp module;
    ...
    public class AlphaBinder extends Binder {
        public AlphaWalletService getService() {
            return AlphaWalletService.this;
        }
    }
        ...
        public void onBlocksDownloaded(final Peer peer, final Block block, final FilteredBlock filteredBlock, final int blocksLeft) {
            ...
            /*if (AlphaContext.IS_TEST)
                ...
                if (now - lastMessageTime > TimeUnit.SECONDS.toMillis(6)) {
                    ...
                    alphaApplication.getAppConf().setLastBestChainBlockTime(block.getTime().getTime());
    ...
    private class RunnableBlockChecker implements Runnable{
        ...
        @Override
        public void run() {
            org.sendj.core.Context.propagate(AlphaContext.CONTEXT);
        ...
        public void onCoinsReceived(Wallet wallet, Transaction transaction, Coin coin, Coin coin1) {
            ...
            try {
                ...
                if (amount.isGreaterThan(Coin.ZERO)) {
                    ...
                    Intent resultIntent = new Intent(getApplicationContext(), AlphaWalletService.this.getClass());
                    resultIntent.setAction(ACTION_CANCEL_COINS_RECEIVED);
                    deleteIntent = PendingIntent.getService(AlphaWalletService.this, 0, resultIntent, PendingIntent.FLAG_CANCEL_CURRENT);
                    mBuilder = new NotificationCompat.Builder(getApplicationContext())
                            .setContentTitle("BLCR received!")
                            ...
                            .setColor(
                                    (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) ?
                                            ...
                                            :
                                            ContextCompat.getColor(AlphaWalletService.this, R.color.bgPurple))
    ...
    public void onCreate() {
        ...
        super.onCreate();
        try {
            log.info("Alpha service started");
            // Android stuff
            ...
            module = (AlphaModuleImp) alphaApplication.getModule();
            blockchainManager = module.getBlockchainManager();
            // connect to bwktrum node
            /*bwktrumPeergroup = new BwktrumPeergroup(alphaApplication.getNetworkConf());
            ...
            String filename = AlphaContext.Files.BLOCKCHAIN_FILENAME;
            boolean fileExists = new File(file,filename).exists();
            blockchainStore = new SnappyBlockchainStore(AlphaContext.CONTEXT,file,filename);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        log.info("Alpha service onStartCommand");
        try {
            if (intent != null) {
                try {
    ...
    private void tryScheduleService() {
        ...
        if (!isSchedule){
            ...
            Intent intent = new Intent(this, AlphaWalletService.class);
    ...
    private void requestRateCoin(){
        final AppConf appConf = alphaApplication.getAppConf();
        AlphaRate alphaRate = module.getRate(appConf.getSelectedRateCoin());
        if (alphaRate==null || alphaRate.getTimestamp()+AlphaContext.RATE_UPDATE_TIME<System.currentTimeMillis()){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        CoinMarketCapApiClient c = new CoinMarketCapApiClient();
                        BigDecimal usdValue = c.getAlphaPrice();
                        AlphaRate alphaRate = new AlphaRate(CoinTypes.USD.name(),usdValue,System.currentTimeMillis(),"coinmarketcap");
                        module.saveRate(alphaRate);
                    } catch (RequestAlphaRateException e) {
                        e.printStackTrace();
                    } catch (Exception e){
                        e.printStackTrace();
                    }
