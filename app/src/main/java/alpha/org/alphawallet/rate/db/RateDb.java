package alpha.org.alphawallet.rate.db;
...
import alpha.org.alphawallet.contacts.AbstractSqliteDb;

/**
 * Created by kaali on 7/5/17.
 */

public class RateDb extends AbstractSqliteDb<AlphaRate> {
    ...
    @Override
    protected ContentValues buildContent(AlphaRate obj) {
    ...
    @Override
    protected AlphaRate buildFrom(Cursor cursor) {
        ...
        return new AlphaRate(coin,value,timestap,link);
    }
    ...
    public void insertOrUpdateIfExist(AlphaRate alphaRate) {
        if (getRate(alphaRate.getCoin())==null){
            insert(alphaRate);
        }else {
            updateByKey(KEY_COIN,alphaRate.getCoin(),alphaRate);
        }
    }
}
