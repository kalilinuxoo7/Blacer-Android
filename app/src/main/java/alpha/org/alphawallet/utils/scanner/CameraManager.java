...
package alpha.org.alphawallet.utils.scanner;
...
public final class CameraManager
{
	...
	public Camera open(final TextureView textureView, final int displayOrientation, final boolean continuousAutoFocus) throws IOException
	{
		...
		Log.i(TAG,"opening camera id {}: {}-facing, camera orientation: {}, display orientation: {}, camera id: "+ cameraId + " "+
		...
		catch (final RuntimeException x)
		{
			if (savedParameters != null)
			{
				...
				{
					Log.i(TAG,"problem setting camera parameters", x2);
				}
			}
		}
		...
		{
			Log.w(TAG,"something went wrong while starting camera preview", x);
			camera.release();
			throw x;
		}
	}
	...
	public void close()
	{
		if (camera != null)
		{
			...
			{
				Log.w(TAG,"something went wrong while stopping camera preview", x);
			}
	...
	public void requestPreviewFrame(final PreviewCallback callback)
	{
		...
		{
			Log.w(TAG,"problem requesting preview frame, callback won't be called", x);
		}
	}
