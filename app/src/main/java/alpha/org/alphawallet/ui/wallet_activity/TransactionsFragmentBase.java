...
import alpha.org.alphawallet.ui.base.BaseRecyclerFragment;
...
import alpha.org.alphawallet.ui.transaction_detail_activity.TransactionDetailActivity;

import static alpha.org.alphawallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static alpha.org.alphawallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static alpha.org.alphawallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = alphaModule.listTx();
    ...
    protected BaseRecyclerAdapter<TransactionWrapper, ? extends BaseRecyclerViewHolder> initAdapter() {
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (alphaRate!=null) {
                    localCurrency = alphaApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * alphaRate.getValue().doubleValue()).movePointLeft(8)
                                    )
                                    + " " + alphaRate.getCoin();
                ...
                holder.title.setText(getAddressOrContact(alphaModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(alphaModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(alphaModule.getConf().getNetworkParams()).toBase58());
                }*/
