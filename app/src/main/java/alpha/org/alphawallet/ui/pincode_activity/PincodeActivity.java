package alpha.org.alphawallet.ui.pincode_activity;
...
import alpha.org.alphawallet.ui.backup_mnemonic_activity.MnemonicActivity;
import alpha.org.alphawallet.ui.base.BaseActivity;
import alpha.org.alphawallet.ui.settings_pincode_activity.KeyboardFragment;
import alpha.org.alphawallet.ui.start_activity.StartActivity;

import static alpha.org.alphawallet.ui.backup_mnemonic_activity.MnemonicActivity.INTENT_EXTRA_INIT_VIEW;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (alphaApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
    ...
    private void goNext() {
        if (alphaApplication.getAppConf().getTrustedNode()==null){
            // select random trusted node
            List<BwktrumPeerData> nodes = BwktrumGlobalData.listTrustedHosts();
            Random random = new Random();
            alphaApplication.setTrustedServer(nodes.get(random.nextInt(nodes.size())));
            alphaApplication.stopBlockchain();
        }

        alphaApplication.getAppConf().setAppInit(true);
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        alphaApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(alphaApplication.getAppConf().getPincode().equals(pincode)){
    ...
    public void onBackPressed() {
        super.onBackPressed();
        // todo: controlar esto
        if (alphaApplication.getAppConf().getPincode()==null){
