...
import alpha.org.alphawallet.ui.contacts_activity.ContactsActivity;
import alpha.org.alphawallet.ui.donate.DonateActivity;
import alpha.org.alphawallet.ui.settings_activity.SettingsActivity;
import alpha.org.alphawallet.ui.wallet_activity.WalletActivity;

import static alpha.org.alphawallet.module.AlphaContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends AlphaActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = alphaApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = alphaModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (alphaModule.getChainHeight()+10>peerHeight) {
            ...
            if (alphaModule.isAnyPeerConnected()) {
                long peerHeight = alphaModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (alphaModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
    ...
    private double calculateBlockchainSyncProgress() {
        long nodeHeight = alphaModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (alphaModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }
